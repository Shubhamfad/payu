...
class Form extends CI_Controller {
	...
	public function index($id=null){	
		if ($id) 
		{
			...
			$data['is_new'] = $data['school']['is_new'];    //1
			
			$str = $data['school']['name'];    //Mahindra Intl School, Pune	
			$ret = '';
			foreach(explode(' ', $str) as $word){
				$ret .= strtoupper($word[0]);
				}
			$data['school_initials'] = $ret;   //MAHINDRA INTL SCHOOL, PUNE
			...
			if ($_POST)
			{
				...
				$formType = $_POST['type'];
				log_message('debug', 'Form Type: ' . print_r($formType, true));
				unset($_POST['type']);
